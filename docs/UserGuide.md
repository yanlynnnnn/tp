...
* Editing a service in the service manager will not change the details of any appointments associated with that service as they are
 kept as separate records, and a service's details should not change if its already agreed upon with the client when making the appointment.
 Edit the appointment after editing the service if you want the service's edited details to be reflected.
